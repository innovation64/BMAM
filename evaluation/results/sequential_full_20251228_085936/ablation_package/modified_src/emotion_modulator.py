...
# 🔥 2025-12-30: 消融实验支持
def _is_salience_enabled() -> bool:
    """检查 Salience (情绪显著性加权) 是否启用"""
    try:
        from ..config.ablation_config import is_component_enabled
        return is_component_enabled('salience')
    except ImportError:
        return True  # 默认启用
...
class EmotionModulator:
    ...
    def modulate_importance(self, base_importance: float, emotion_intensity: float) -> float:
        ...
        """
        # 🔥 2025-12-30: 消融实验检查
        if not _is_salience_enabled():
            # 禁用时不做任何情绪加权，直接返回基础重要性
