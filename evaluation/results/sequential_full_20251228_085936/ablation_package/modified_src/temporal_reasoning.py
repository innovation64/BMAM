...
# 🔥 2025-12-30: 消融实验支持
def _is_temporal_reasoning_enabled() -> bool:
    """检查 Temporal Reasoning 是否启用"""
    try:
        from ....config.ablation_config import is_component_enabled
        return is_component_enabled('temporal_reasoning')
    except ImportError:
        return True  # 默认启用
...
class TemporalReasoningMixin:
    ...
    async def _temporal_reasoning(
        ...
        # 🔥 2025-12-30: 消融实验检查
        if not _is_temporal_reasoning_enabled():
            logger.warning("⚠️ 消融实验: Temporal Reasoning 已禁用")
            return {
                'answer': None,
                'confidence': 0.0,
                'reasoning_chain': ['Temporal reasoning disabled (ablation experiment)'],
                'source': 'ablation_disabled'
            }
