    ...
    async def store_memory_with_timestamp(
        ...
        if auto_chunk and estimated_tokens > chunk_threshold:
            ...
            for i, chunk in enumerate(chunks):
                ...
                # 🔥 2025-12-31: 消融实验支持 - 处理 hippocampus 被禁用的情况
                if result is None:
                    result = {
                        'memory_id': None,
                        'status': 'skipped',
                        'reason': 'hippocampus_disabled_ablation'
                    }
...
简要总结文档的核心主题和关键信息。"""
    ...
    async def smart_retrieve(
        ...
        try:
            ...
            # 🔥 2025-12-31 FIX: 只对事件/时间线相关查询触发 StoryArc 增强，避免干扰简单查询
            try:
                augmented = False
                ...
                story_arc_enabled = is_component_enabled('story_arc') and self.story_arc

                # 只对事件/时间线相关查询触发 StoryArc
                query_lower = query.lower()
                event_keywords = [
                    'when did', 'what happened', 'event', 'timeline', 'history',
                    'first time', 'last time', 'sequence', 'order of', 'before', 'after',
                    'how long', 'how many times', 'experience'
                ]
                is_event_query = any(kw in query_lower for kw in event_keywords)

                if story_arc_enabled and is_event_query:
                    # 提取查询中的实体
