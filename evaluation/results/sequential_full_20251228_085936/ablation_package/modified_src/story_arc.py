...
# 🔥 2025-12-30: 消融实验支持
def _is_story_arc_enabled() -> bool:
    """检查 StoryArc 是否启用（消融实验支持）"""
    try:
        from ..config.ablation_config import is_component_enabled
        return is_component_enabled('story_arc')
    except ImportError:
        return True  # 默认启用
...
class DisabledStoryArcManager:
    """消融实验: 禁用的 StoryArc 占位符"""

    def add_event(self, *args, **kwargs) -> Optional[str]:
        return None

    def query_event_time(self, *args, **kwargs) -> Optional[Dict]:
        return None

    def query_events_by_entity(self, *args, **kwargs) -> List:
        return []

    def get_events_in_range(self, *args, **kwargs) -> List:
        return []

    def calculate_duration(self, *args, **kwargs) -> Optional[Dict]:
        return None

    def get_timeline_context(self, *args, **kwargs) -> str:
        return ""

    async def add_event_from_memory(self, *args, **kwargs) -> Optional[Any]:
        """消融实验: 不添加事件"""
        return None

    def get_entity_context(self, *args, **kwargs) -> List:
        """消融实验: 返回空实体上下文"""
        return []

    def get_stats(self) -> Dict:
        return {'disabled': True, 'reason': 'ablation_experiment'}
...
# 禁用占位符单例
_disabled_story_arc_manager: Optional[DisabledStoryArcManager] = None
...
def get_story_arc_manager(data_dir: Optional[Path] = None) -> StoryArcManager:
    ...
    🔥 2025-12-30: 消融实验支持
    如果 enable_story_arc=False，返回禁用的占位符
    """
    global _story_arc_manager, _disabled_story_arc_manager

    # 消融检查
    if not _is_story_arc_enabled():
        if _disabled_story_arc_manager is None:
            _disabled_story_arc_manager = DisabledStoryArcManager()
            logger.warning("⚠️ 消融实验: StoryArc 已禁用，返回占位符")
        return _disabled_story_arc_manager
...
def reset_story_arc_manager():
    ...
    _disabled_story_arc_manager = None  # 🔥 也重置禁用占位符
