...
# 🔥 2025-12-30: 消融实验支持
def _is_hybrid_retrieval_enabled() -> bool:
    """检查 Hybrid Retrieval 是否启用"""
    try:
        from ...config.ablation_config import is_component_enabled
        return is_component_enabled('hybrid_retrieval')
    except ImportError:
        return True  # 默认启用
...
class HybridSearchMixin:
    ...
    async def hybrid_search(
        ...
        **filters
        ...
        """
        # 🔥 2025-12-30: 消融实验检查
        if not _is_hybrid_retrieval_enabled():
            logger.warning("⚠️ 消融实验: Hybrid Retrieval 已禁用，回退到纯语义搜索")
            # 禁用时回退到纯语义搜索，不应用元数据过滤
            semantic_results = await self.semantic_search(query, k, threshold)
            for r in semantic_results:
                r['search_type'] = 'semantic_only_ablation'
