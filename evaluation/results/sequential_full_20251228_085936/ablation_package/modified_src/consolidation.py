...
# 🔥 2025-12-30: 消融实验支持
def _is_consolidation_enabled() -> bool:
    """检查 Consolidation 是否启用"""
    try:
        from ....config.ablation_config import is_component_enabled
        return is_component_enabled('consolidation')
    except ImportError:
        return True  # 默认启用
...
class ConsolidationAgent(
    ...
    async def process_message(self, message: AgentMessage) -> Dict[str, Any]:
        ...
        # 🔥 2025-12-30: 消融实验检查
        if not _is_consolidation_enabled():
            logger.warning("⚠️ 消融实验: Consolidation 已禁用")
            return {
                'status': 'skipped',
                'reason': 'consolidation_disabled_ablation',
