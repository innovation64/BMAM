...
class HRMCoordinatorWrapper:
    ...
    async def process_user_input(self, user_input: str, context: Dict[str, Any] = None) -> Any:
        ...
        # 🔥 2025-12-31: 消融实验支持 - 检查 HRM 是否启用
        try:
            from ..config.ablation_config import is_component_enabled
            hrm_enabled = is_component_enabled('hrm')
        except ImportError:
            hrm_enabled = True

        # 添加 HRM 配置到上下文
        context['hrm_config'] = {
            'enable_multi_timescale': self.config.enable_multi_timescale and hrm_enabled,
            'enable_act': self.config.enable_act and hrm_enabled,
            'max_iterations': self.config.max_iterations if hrm_enabled else 1,
            'convergence_threshold': self.config.convergence_threshold,
            'confidence_threshold': self.config.confidence_threshold,
            'hrm_ablation_disabled': not hrm_enabled  # 标记消融状态
        }

        if not hrm_enabled:
            logger.info("⚠️ 消融实验: HRM 已禁用，跳过多时间尺度协调和 ACT")
