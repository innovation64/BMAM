...
async def run_locomo_ablation(
    ...
    for sample_idx, sample in enumerate(samples):
        ...
        try:
            ...
            print(f" 塑造 {len(sessions)} sessions", end='', flush=True)
            for idx, (dialogs, date, ob) in enumerate(sessions):
                ts = parse_date(date)
                print(f"\r  [{sample_idx+1}/{len(samples)}] {sample['sample_id']} Session {idx+1}/{len(sessions)}", end='', flush=True)
            ...
            print(f"  QA测试: {total_qa} 题", flush=True)

            for qa_idx, qa in enumerate(sample['qa'], 1):
                ...
                # 实时进度
                current_acc = sample_correct / qa_idx * 100
                print(f"\r  {qa_idx}/{total_qa} {'✓' if ok else '✗'} acc={current_acc:.1f}%", end='', flush=True)

            correct_total += sample_correct
            question_total += len(sample['qa'])

            acc = sample_correct / len(sample['qa']) * 100 if sample['qa'] else 0
            print(f"\n  → 结果: {sample_correct}/{len(sample['qa'])} ({acc:.1f}%)", flush=True)
...
async def main():
    ...
    parser.add_argument('--log', type=str, default=None,
                       help='实时日志文件路径 (默认: results/ablation/ablation_live.log)')
    ...
    if args.log:
        ...
        log_file = RESULTS_DIR / 'ablation_live.log'
