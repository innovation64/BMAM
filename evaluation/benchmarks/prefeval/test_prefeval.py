...
async def test_sample(coord, sample, client, idx, total):
    ...
    # 提问
    await _tracer.async_start_query(question, query_type='preference')
    ...
    await _tracer.async_end_query(answer=response, correct=is_correct, expected_answer=preference)
...
async def main():
    ...
    tracer = get_tracer()
    tracer.print_report()
    tracer.save_report(str(RESULTS_DIR / f'pipeline_trace_{ts}.json'))
