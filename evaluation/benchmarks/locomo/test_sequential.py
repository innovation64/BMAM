...
LOCOMO_PATH = Path(_locomo_env) if _locomo_env else PROJECT_ROOT / 'data' / 'datasets' / 'locomo' / 'locomo10.json'
DATA_DIR = PROJECT_ROOT / 'data' / 'memory'
RESULTS_DIR = PROJECT_ROOT / 'evaluation' / 'results' / 'sequential'
...
async def main():
    ...
    p.add_argument('--start-group', type=int, default=0, help='起始组索引 (0-9)')
    p.add_argument('--questions', default='all')
    p.add_argument('--skip-shaping', action='store_true', help='跳过记忆塑造，直接测试QA')
    args = p.parse_args()

    start_group = max(0, min(9, args.start_group))
    num_groups = min(max(1, args.groups), 10 - start_group)
    num_q = 'all' if args.questions == 'all' else int(args.questions)
    skip_shaping = args.skip_shaping

    print("=" * 60)
    print(f"LoCoMo 顺序测试: {num_groups} 组 (从第{start_group}组开始), 每组 {num_q} 题" + (" (跳过塑造)" if skip_shaping else ""))
    print("=" * 60)
