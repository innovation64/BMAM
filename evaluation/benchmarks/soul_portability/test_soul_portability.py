...
async def phase3_restore(archive_path: str) -> Tuple[bool, int, List[str], Any]:
    """Phase 3: 清空并恢复记忆。返回 (success, count, regions, coordinator)"""
    print("\n🔄 Phase 3: 清空并恢复记忆")
    print("-" * 40)
    ...
    try:
        ...
        brain_regions = list(brain_regions_data.keys()) if brain_regions_data else []

        print(f"  ✓ 恢复记忆: {memories_count} 条")
        print(f"  ✓ 恢复脑区: {brain_regions}")

        return True, memories_count, brain_regions, coord
...
async def run_soul_portability_test(
    ...
    try:
        ...
        # Phase 3: 清空并恢复（返回已恢复记忆的 coordinator）
        success, restore_count, restored_regions, restored_coord = await phase3_restore(archive_path)
        ...
        # 使用已恢复记忆的 coordinator（不要重新创建空白的）
        coord = restored_coord
