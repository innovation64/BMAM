    ...
    for i, (session_dialogues, session_date, session_obs) in enumerate(sessions_data, 1):
        ...
        for turn in session_dialogues:
            ...
            if text:
                ...
                await coordinator.store_memory_with_timestamp(
                    content=content,
                    timestamp=timestamp,
                    speaker=speaker,
                    importance=0.6
        ...
        processed_obs += obs_count

        # 更新进度
        print_progress_bar(
            i, total_sessions,
            prefix='  塑造进度',
            suffix=f'Session {i} ({turn_count} turns, {obs_count} obs)'
        )
...
async def test_single_sample(sample_idx, sample, num_questions, llm_client, num_samples, skip_ingestion=False) -> Dict:
    ...
    # 创建 coordinator (与消融脚本一致，使用 HRM wrapper)
    ...
    try:
        ...
        coordinator = HRMCoordinatorWrapper(base_coordinator, hrm_config)
    except ImportError:
