...
class AmygdalaAgent(BrainAgent):
    ...
    async def modulate_memory_encoding(
        ...
        try:
            ...
            # 如果情绪强度很高,触发海马体的优先巩固
            _ecfg = get_config().emotion
            consolidation_triggered = False
            if emotion_intensity >= _ecfg.high_intensity_threshold and self.temporal_lobe:
                # 通知海马体这是高情绪记忆,应该优先巩固
    ...
    async def trigger_stress_response(
        ...
        try:
            ...
            # 高压下的生理效应
            if self.current_stress_level >= _ecfg.high_intensity_threshold:
            ...
            elif self.current_stress_level >= _ecfg.moderate_arousal_threshold:
