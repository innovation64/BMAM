...
class VoiceAnimeUI:
    ...
    async def process_text_input(self, text: str) -> str:
        """Process already-transcribed text through BMAM (skips STT)."""
        self.is_listening = True
        self.character.set_state(CharacterState.LISTENING)
        self.waveform.set_active(True)

        # Show user input in chat bubble
        self.chat_bubble.show_message(f"You: {text}")

        # React to input
        self.character.react_to_input(text)
    ...
    async def process_text_input_full(self, text: str) -> dict:
        """Process text and return response with brain activity data."""
        self.is_listening = True
        self.character.set_state(CharacterState.LISTENING)
        self.waveform.set_active(True)
        ...
        # Build brain activity data from the result
        regions = {}
        if hasattr(result, 'agents_involved') and result.agents_involved:
            for agent_id in result.agents_involved:
                regions[agent_id.lower().replace(' ', '_')] = 1.0

        reasoning_chain = []
        if hasattr(result, 'activation_trace') and result.activation_trace:
            reasoning_chain = result.activation_trace
        elif hasattr(result, 'agent_logs') and result.agent_logs:
            for agent_name, logs in result.agent_logs.items():
                for log in logs:
                    reasoning_chain.append({
                        'agent': agent_name,
                        'type': log.get('type', 'processing'),
                        'description': log.get('message', log.get('detail', 'Processing...')),
        ...
        return {
            ...
            } if regions or reasoning_chain else None,
        }
