...
async def main():
    ...
    print("🚀 Initializing BMAM Brain-Inspired Coordinator...")
    coordinator = BrainInspiredCoordinator()
    await coordinator.initialize()

    # Create and start web server
    server = WebUIServer(coordinator, port=8081)

    # Wire up the voice service so WebSocket handlers can use it
    from src.services.voice_service import VoiceService
    server._voice_service = VoiceService()

    if await server.start():
        print(f"Voice Anime UI running at http://localhost:{server.port}")
