...
class BrainInspiredCoordinator:
    ...
    # Unknown-answer patterns. Lowercased substrings; presence of any of them in
    # a short response means the system effectively gave up. Kept conservative
    # — long answers that incidentally contain "not mentioned" should NOT match.
    ...
    # Temporal markers — if the question carries any of these, it's a temporal
    # question and we do NOT run the unknown rescue (per design review #temporal
    # past-fix lessons: temporal handling is its own subsystem and shouldn't be
    # patched at the answer-shaping layer). Kept narrow on purpose — generic
    # words like 'before/after/during' appear in non-temporal questions
    # ("what did she realize after the race?") and over-blocking would defeat
    # the rescue.
    ...
    @classmethod
    def _looks_like_unknown_answer(cls, answer: str) -> bool:
        """True iff `answer` is a short response matching an unknown-pattern."""
        if not answer:
            return True
        # Don't treat a long, substantive answer as unknown just because it
        # happens to contain "not mentioned" deep in a paragraph.
        if len(answer.split()) > 12:
            return False
        a = answer.strip().strip('"').strip("'").strip('.').lower()
        return any(p in a for p in cls._UNKNOWN_ANSWER_PATTERNS)

    @classmethod
    def _query_is_temporal(cls, query: str) -> bool:
        """Heuristic: does this query ask about time?"""
        if not query:
            return False
        q = (' ' + query.strip().lower())
        if q.lstrip().startswith('when'):
            return True
        return any(m in q for m in cls._TEMPORAL_QUERY_MARKERS)

    async def _unknown_answer_rescue(
        self,
        query: str,
        answer: str,
        memories: list,
        ...
        """
        Evidence-gated unknown rescue.

        Trigger conditions (BOTH must hold, else return answer unchanged):
          1. `answer` matches an unknown-pattern (system gave up)
          2. `query` is non-temporal (let temporal stay temporal)

        Behaviour:
          - Sends the already-retrieved `memories` to the LLM with strict
            extraction rules.
          - LLM is required to either output a concrete, quotable answer
            from the memories OR the literal token UNKNOWN.
          - If the LLM still returns UNKNOWN (or anything matching an
            unknown-pattern), the original answer is preserved.
          - If the LLM returns something specific, it replaces the answer.

        This is intentionally NOT a confidence-floor change. It only fires
        on the narrow surface where the system already declined.
        """
        if not self._looks_like_unknown_answer(answer):
            return answer
        if self._query_is_temporal(query):
            return answer
        if not memories or len(memories) < 2:
            return answer

        # Materialise memory snippets. Accept dicts and pydantic-ish objects.
        snippets: list = []
        ...
        memory_block = '\n---\n'.join(snippets)
        prompt = (
            "Extract a concrete answer from the MEMORY snippets for the QUESTION.\n\n"
            "RULES (strict):\n"
            "1. If the memories contain a clear, direct answer, output ONLY that "
            "answer (max 10 words, no preamble, no explanation).\n"
            "2. If the memories do not contain a clear answer, output exactly: UNKNOWN\n"
            "3. Do NOT invent facts. Do NOT generalize. The answer must be quotable "
            "or directly inferable from a single memory.\n"
            "4. Never output 'Based on memories...' or 'The answer is...'.\n\n"
            f"QUESTION: {query}\n\n"
            f"MEMORIES:\n{memory_block}\n\n"
            "ANSWER:"
        )
        ...
        try:
            ...
            class _TempRescuer(BrainAgent):
                async def process_message(self, msg):
                    return {}

            rescuer = _TempRescuer(
                agent_id='unknown_rescuer',
                brain_region='prefrontal',
                system_prompt='Unknown Answer Rescuer',
            )
            rescued = await rescuer.call_llm(prompt, temperature=0.0, max_tokens=40)
            rescued = (rescued or '').strip().strip('"').strip("'").strip('.')
        except Exception as e:
            logger.debug(f"Unknown rescue LLM call skipped: {e}")
            return answer

        if not rescued:
            return answer
        if rescued.upper() == 'UNKNOWN':
            return answer
        if len(rescued.split()) > 12:
            return answer
        if self._looks_like_unknown_answer(rescued):
            return answer

        _trace_agent('unknown_rescuer')
        logger.info(f"🛟 Unknown rescue: {answer[:40]!r} → {rescued[:60]!r}")
        return rescued
...
Output ONLY the extracted answer:"""
    ...
    async def _process_user_input_impl(self, user_input: str, context: Dict[str, Any] = None) -> ProcessingResult:
        ...
        try:
            ...
            # Unknown-answer rescue: if `response` looks like the system gave
            # up ("Information not available" / "unknown" / "not mentioned"…)
            # AND the question is non-temporal, attempt one more strict
            # extraction from the already-retrieved memories. Replaces only
            # when an evidence-quotable answer comes back; preserves the
            # original 'unknown' otherwise. See _unknown_answer_rescue.
            try:
                response = await self._unknown_answer_rescue(
                    user_input, response, memories
                )
            except Exception as e:
                logger.debug(f"Unknown rescue wrapper skipped: {e}")
