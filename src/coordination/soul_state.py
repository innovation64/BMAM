...
class SoulState:
    ...
    def add_thought(self, thought: str):
        """Add a thought to the stream (thread-safe, sync)"""
        with self._sync_lock:
            self._add_thought_unsafe(thought)

    async def async_add_thought(self, thought: str):
        """Add a thought to the stream (async, for use in async contexts)"""
        async with self._get_async_lock():
            self._add_thought_unsafe(thought)

    def get_state(self) -> Dict[str, Any]:
        """Get current state snapshot (thread-safe, sync)"""
        with self._sync_lock:
            return self._get_state_unsafe()

    async def async_get_state(self) -> Dict[str, Any]:
        """Get current state snapshot (async, for use in async contexts)"""
        async with self._get_async_lock():
            return self._get_state_unsafe()

    def _get_state_unsafe(self) -> Dict[str, Any]:
        """Get state dict without acquiring lock (must be called while holding lock)"""
        return {
            'emotion': self.current_emotion,
            ...
            'user_profile_summary': {
                ...
                'goals_count': (len(self.user_profile.short_term_goals)
                                + len(self.user_profile.long_term_goals)),
    ...
    def _record_insight_unsafe(
        ...
        """Core insight recording logic (must be called while holding lock)"""
    ...
    def record_insight(
        ...
        """
        with self._sync_lock:
            return self._record_insight_unsafe(
                trigger_type, trigger_reason, conclusion,
                follow_up_actions, confidence, source_query,
                related_memory_ids
            )

    async def async_record_insight(
        self,
        trigger_type: str,
        trigger_reason: str,
        conclusion: str,
        ...
        """
        记录一个洞察/自省结果 (async, for use in async contexts)
    ...
    async def async_record_failure(self, query: str, reason: str, follow_up: str = None):
        """快捷方法: 记录失败 (async)"""
        return await self.async_record_insight(
            trigger_type='failure',
            trigger_reason=f"Query failed: {query[:50]}",
            conclusion=reason,
            follow_up_actions=[follow_up] if follow_up else ['Review query handling'],
    ...
    async def async_record_low_confidence(
    ...
    def get_recent_insights(self, count: int = 10, trigger_type: str = None) -> List[Dict]:
        """获取最近的洞察记录 (sync)"""
        with self._sync_lock:
            return self._get_recent_insights_unsafe(count, trigger_type)

    async def async_get_recent_insights(
        self, count: int = 10, trigger_type: str = None
    ) -> List[Dict]:
        """获取最近的洞察记录 (async)"""
        async with self._get_async_lock():
            return self._get_recent_insights_unsafe(count, trigger_type)

    def _get_recent_insights_unsafe(
        self, count: int = 10, trigger_type: str = None
    ) -> List[Dict]:
        """Core recent insights logic (must be called while holding lock)"""
    ...
    def generate_introspection_summary(self) -> Dict[str, Any]:
        """
        生成自省摘要 - 汇总最近的失败/低置信度/冲突 (sync)

        Returns:
            包含自省摘要的字典
        """
        with self._sync_lock:
            return self._generate_introspection_summary_unsafe()

    async def async_generate_introspection_summary(self) -> Dict[str, Any]:
        """
        生成自省摘要 - 汇总最近的失败/低置信度/冲突 (async)
    ...
    def _generate_introspection_summary_unsafe(self) -> Dict[str, Any]:
        """Core introspection summary logic (must be called while holding lock)"""
        now = datetime.now()
        ...
        if len(recent_failures) > 3:
            summary['top_concerns'].append(
                f"High failure rate: {len(recent_failures)} recent failures"
            )
        if len(recent_conflicts) > 2:
            summary['top_concerns'].append(
                f"Memory conflicts detected: {len(recent_conflicts)} conflicts"
            )
        if len(low_confidence) > 5:
            summary['top_concerns'].append(
                f"Decision uncertainty: {len(low_confidence)} low-confidence decisions"
            )

        # 添加到思考流
        concern_text = ("; ".join(summary['top_concerns'])
                        if summary['top_concerns'] else "No major concerns")
