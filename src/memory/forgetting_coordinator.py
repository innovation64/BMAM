...
class ForgettingCoordinator:
    ...
    def _calculate_forgettability(self, advice: MemoryProtectionAdvice) -> float:
        ...
        # Weighted average (weights from centralized config)
        _fcfg = get_config().forgetting
        forgettability = (
            importance_factor * _fcfg.importance_weight +  # Importance is key
            access_factor * _fcfg.access_weight +          # Access frequency matters
            age_factor * _fcfg.age_weight +                # Age contributes
            emotion_factor * _fcfg.emotion_weight          # Emotion factor
        )
