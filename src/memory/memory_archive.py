    ...
    def create_from_coordinator(
        ...
        if hasattr(coordinator, 'hippocampus') and coordinator.hippocampus:
            try:
                ...
                hippo_memory_count = len(hippo_state.get('memories', []))
                brain_regions_info['hippocampus'] = {
                    'file': 'brain_regions/hippocampus.json',
                    'format': 'json',
                    'required': False,
                    'memory_count': hippo_memory_count
                }
                total_memories += hippo_memory_count
                logger.info(f"   ✓ Exported Hippocampus: {hippo_memory_count} memories")
